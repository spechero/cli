Feature: Problem sharing

    Background:
        Given the spechero cli has been installed
        And "bob" has no existing spechero specs
        And "bob" is authenticated to his spechero account
        And "mary" has no existing spechero specs
        And "mary" is authenticated to her spechero account

    Scenario: A problem is solved successfully
    ...
    Scenario: A solution is rejected

    Scenario: A problem is closed without a solution
